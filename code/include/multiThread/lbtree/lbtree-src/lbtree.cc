...
/********************add by hpy***********************************/
#ifdef SEARCH_PERF
uint64_t time_search_dram, time_search_pmem;
#endif
/********************************************************/
...
#ifndef TREE_NO_BUFFER
#define OPEN_ENTRY_MOVING
#endif

#ifdef OPEN_ENTRY_MOVING
...
inline bleaf *alloc_lnode()
{
#ifndef MEMPOOL
    TOID(bleaf)
    leaf = TOID_NULL(bleaf);
    POBJ_ZNEW(pop, &leaf, bleaf);
...
#else
    // printf("%d\n",sizeof(bleaf));
    return (bleaf *)nvmpool_alloc(sizeof(bleaf));

#endif
}

inline bnode *alloc_inode()
{
    ...
    if (node == NULL)
    {
        fprintf(stderr, "failed to create a btree node in dram.\n");
        exit(0);
    }
    return node;
}

lbtree::lbtree(bool recover = false)
{
    initUseful();
#ifndef MEMPOOL
    TOID(bleaf)
    root = POBJ_ROOT(pop, bleaf);
    assert(!TOID_IS_NULL(root));
    // create the first leaf. and set the btree's root.
    tree_meta = new treeMeta((void *)D_RW(root), recover);
#else
    void *root = (void *)alloc_lnode();
    tree_meta = new treeMeta(root, recover);
#endif
    if (!tree_meta)
    {
        perror("new");
...
/*NOTE(by zhenxin): After removing the pool design, lbtree can not use bulkload.
    ...
    Pointer8B pfirst[], int n_nodes[])
{
    ...
    leaf_fill_num= lbtree_max(leaf_fill_num, 1);

    int nonleaf_fill_num= (int)((float)NON_LEAF_KEY_NUM * bfill);
    nonleaf_fill_num= lbtree_max(nonleaf_fill_num, 1);
...
#ifdef SEARCH_PERF
    time_search_dram += ElapsedNanos(time_start1);

    uint64_t time_start2 = NowNanos();
#endif
...
#ifdef SEARCH_PERF
    time_search_pmem += ElapsedNanos(time_start1);
#endif
...
void lbtree::insert(key_type key, void *ptr, bool update)
{
    ...
    Again2:
        ...
        while (mask)
        {
            ...
            {               // found: do nothing, return
                if (update) // update ops, overwrite
                {
                    // found: update
                    lp->lock = 1;
#ifdef OPEN_RTM
...
#endif
                    lp->ent[jj].ch = ptr;
                    clflush(&lp->ent[jj].ch, 8);
                    lp->lock = 0;
                    clflush(lp,8);
                    return;
                }
                else // insert ops, found the same key ,just return.
                {
#ifdef OPEN_RTM
                    _xend();
#endif
...
//去掉entry moving
        ...
        hist_set_group[thread_id]->Add(DO_SPLIT_1, ElapsedNanos(time_start));
        time_start = NowNanos();
#endif
        // 2.3 create new node
        // bleaf * newp = (bleaf *)alloc_lnode(LEAF_SIZE);
        bleaf *newp = (bleaf *)alloc_lnode();
// 2.4 move entries sorted_pos[split .. LEAF_KEY_NUM-1]
#ifdef SPLIT_BREAKDOWN
        hist_set_group[thread_id]->Add(DO_SPLIT_2, ElapsedNanos(time_start));
        time_start = NowNanos();
#endif
...
#ifdef SPLIT_BREAKDOWN
    hist_set_group[thread_id]->Add(DO_SPLIT_3, ElapsedNanos(time_start));
    time_start = NowNanos();
#endif
    /* Part 3. nonleaf node */
    {
...
#endif
#ifdef SPLIT_BREAKDOWN
                hist_set_group[thread_id]->Add(DO_SPLIT_4, ElapsedNanos(time_start));
#endif
                return;
            }

            /* otherwise allocate a new non-leaf and redistribute the keys */
            //   newp = (bnode *)alloc_inode(NONLEAF_SIZE);
            newp = (bnode *)alloc_inode();
...
void lbtree::del(key_type key)
{
    ...
    Again3:
        ...
        if (lp->num() == 1)
        {
            ...
            {
                p = parray[i];
                p = p->ch(ppos[i] - 1); //-1得到left
                i--;

                for (; i >= 1; i--) //得到left的子孙后代中的最右leaf。也就是待删node的左
                {
                    if (p->lock())
                    {
#ifdef OPEN_RTM
    ...
    /* Part 2. leaf node */
    {
        ...
        if (leaf_sibp != NULL)
        {
            ...
            clflush(&(leaf_sibp->next[0]), 8); // flush the pointer
...
void lbtree::randomize(Pointer8B pnode, int level)
{
    ...
    else
    {
        ...
        for (int i = 0; i < num * 2; i++)
        {
            ...
            if (aa != bb)
            {
                lbtree_swap(lp->fgpt[pos[aa]], lp->fgpt[pos[bb]]);
                lbtree_swap(lp->ent[pos[aa]], lp->ent[pos[bb]]);
            }
        }
    }
}
...
{
    tree *mytree = new lbtree(recover);
    return mytree;
}
...
int lbtree::scan(key_type minkey, uint64_t len, std::vector<value_type_sob> &buf)
{
    bool is_first_node;
    bnode *p;
    bleaf *lp;
    ...
    unsigned char key_hash = hashcode1B(minkey);
    int ret_pos;
    ...
    while (buf.size() < len)
    {
        ...
        for (b = 0; b < LEAF_KEY_NUM; b++)
        {
            if (lp->bitmap & (1 << b))
            {
                if (is_first_node)
                {
                    if (lp->ent[b].k >= minkey)
                        buf.push_back(lp->ent[b].ch);
                }
                else
                {
                    buf.push_back(lp->ent[b].ch);
...
//  寻找leaf中最小的key
inline key_type get_left_key(bleaf *lp)
{
    uint16_t bitmap = lp->bitmap;
    bool is_first_key = true;
    key_type min_key;
    while (bitmap)
    {
        int x = bitScan(bitmap) - 1; // 1是非空
        if (is_first_key)
        {
            min_key = lp->k(x);
            is_first_key = false;
        ...
        {
            min_key = std::min(min_key, lp->k(x));
        }
        bitmap &= ~(1 << x);
    }
    return min_key;
...
int lbtree::recovery_bulkloadSubtree(
    bleaf *firstleaf, int start_key, int num_leaf,
    float bfill, int target_level,
    Pointer8B pfirst[], int n_nodes[]) // bfill 一个节点填充得多满 1为填满
{
    // We assume that a tree cannot be higher than 32 levels
    int ncur[32];  // current node at every level
    int top_level; // top_level is the top level that this method builds
    ...
    // 2. compute number of nodes   每一层需要几个node
    n_nodes[0] = num_leaf;
    top_level = 0;
    ...
    for (int i = 1; i <= top_level; i++)
    {
        pfirst[i] = malloc(sizeof(bnode) * n_nodes[i]);
    }
    ...
    { //叶子数
        ...
        key_type left_key = get_left_key(lp);
 ...
 * if the root level is smaller target_level)
 */
int lbtree::recovery_bulkloadToptree(
    Pointer8B ptrs[], key_type keys[], int num_key,
    float bfill, int cur_level, int target_level,
    Pointer8B pfirst[], int n_nodes[])
{
 ...
 *
 */
void lbtree::recovery_getKeyPtrLevel(
    Pointer8B pnode, int pnode_level, key_type left_key,
    int target_level, Pointer8B ptrs[], key_type keys[], int &num_nodes,
    bool free_above_level_nodes)
{
    ...
    {
        printf("======pnode_level > target_level======\n");

        bnode *p = pnode;
        recovery_getKeyPtrLevel(p->ch(0), pnode_level - 1, left_key,
                                target_level, ptrs, keys, num_nodes,
                                free_above_level_nodes);
        for (int i = 1; i <= p->num(); i++)
        {
            recovery_getKeyPtrLevel(p->ch(i), pnode_level - 1, p->k(i),
                                    target_level, ptrs, keys, num_nodes,
                                    free_above_level_nodes);
        }
...
typedef struct recovery_BldThArgs
{

    Pointer8B first_leaf; // input
    key_type start_key;   // input
    key_type num_key;     // input
...
// bulkload using multiple threads
//
int lbtree::recovery_load(int leafnum, bleaf *firstleaf, float bfill, int worker_thread_num)
{
    // 1. allocate BldThArgs[]
    // int num_threads= ((leafnum>worker_thread_num*10) ?  worker_thread_num : 1);

    recovery_BldThArgs *bta = new recovery_BldThArgs[num_threads];
    if (!bta)
    {
        perror("malloc");
        exit(1);
    ...
    // 3. compute start num_key for each thread
    // int ln_per_thread= floor(leafnum, num_threads);
    // int ln_max= leafnum-(num_threads-1)*ln_per_thread;

    // for (int i=0; i<num_threads; i++) {
    //    bta[i].start_key= i*ln_per_thread;  //start leaf
    //    bta[i].num_key= ((i<num_threads-1)? ln_per_thread : ln_max);   //num leaf
    // }

    int num_threads = worker_thread_num;
    int ln_per_thread = floor(leafnum, num_threads); // lc = log chunk
    int tmp = 0;
    int rest = leafnum - ln_per_thread * num_threads;
    ...
    for (int i = 0; i < num_threads; i++)
    {
        ...
        if (rest)
        {
            bta[i].num_key = ln_per_thread + 1;
            rest--;
        }
        else
        {
            bta[i].num_key = ln_per_thread;
        }
        tmp += bta[i].num_key;
    }

    int cnt = 0;
    bleaf *leaf = firstleaf;
    int now_thread_id = 0;
    // printf("num_threads = %d\n", num_threads);
    while (leaf)
    {
        assert(cnt < leafnum);
        if (cnt == bta[now_thread_id].start_key)
        {
            bta[now_thread_id].first_leaf = leaf;
            now_thread_id++;
            if (now_thread_id == num_threads)
                break;
        }
        leaf = leaf->next[leaf->alt];
        cnt++;
    }
    assert(now_thread_id == num_threads);

    // 0 .. num_threads-2: kn_per_thread keys
    // num_threads-1:      kn_max keys  (kn_max >= kn_per_thread)

    uint64_t time_start = NowNanos(); //////////////////////////s
    ...
    for (int i = 0; i < num_threads; i++)
    {
        threads[i] = std::thread([=]()
                                 {
                       worker_id= i;
                       bta[i].top_level= recovery_bulkloadSubtree(
                               bta[i].first_leaf, bta[i].start_key, bta[i].num_key,
                               bfill, 31,
                               bta[i].pfirst, bta[i].n_nodes); }); // pfirst 每层的第一个节点地址  n_nodes：每层的节点数
    }
    for (int i = 0; i < num_threads; i++)
        threads[i].join();

    // printf("part 1 : %llu ns\n", ElapsedNanos(time_start));
    // time_start = NowNanos();

    // 5. put the ptr to the top nonleaf nodes into an array
    //    using the min top level
    int level = bta[0].top_level; // subtree 0 .. num_threads-2    subtree中最高的
    ...
    for (int i = 0; i < num_threads; i++)
    {
        ...
        //    key_type left_key= lp->k(LEAF_KEY_NUM - lp->num());
        key_type left_key = get_left_key(lp); //每个线程的leaf中的最小值

        recovery_getKeyPtrLevel(bta[i].pfirst[bta[i].top_level], //子树的根
                                bta[i].top_level, left_key,
                                level, top_ptrs, top_keys, num_nodes, true);
    }
    ...
    // printf("part 2 : %llu ns\n", ElapsedNanos(time_start));
    // time_start = NowNanos();

    // 6. build the top nonleaf nodes
    bta[0].top_level = recovery_bulkloadToptree(top_ptrs, top_keys, num_nodes, bfill,
                                                level, 31, bta[0].pfirst, bta[0].n_nodes);
    ...
    // printf("part 3 : %llu ns\n", ElapsedNanos(time_start));
    // time_start = NowNanos();
    printf("%d threads time of recovery from leaf : %llu ns\n", num_threads, ElapsedNanos(time_start));
...
void lbtree::recovery(bleaf *firstleaf, int worker_thread_num)
{
    int leafnum = 0;
    bleaf *ptr = firstleaf;
    while (ptr)
    {
        // printf("leafnum = %d\n", leafnum);
        leafnum++;
        ptr = ptr->next[ptr->alt];
    }
    printf("leafnum = %d\n", leafnum);
    worker_thread_num = 20;
    recovery_load(leafnum, firstleaf, 1, worker_thread_num);
}
