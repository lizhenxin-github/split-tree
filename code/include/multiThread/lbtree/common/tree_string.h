...
// #define MAX_KEY		((key_type)(0xffffffffffffffffULL))
// #define MIN_KEY		((key_type)(0x0000000000000000ULL))

#define LBTREE_DEBUG_INFO(format, ...) do{                            \
      fprintf(stderr, "[%s|%04d]: " format,		 \
         __FILE__, __LINE__, ##__VA_ARGS__); \
   }while(0);

#define LBTREE_RTM_SHOW_TEST()  fprintf(stderr, "[%s|%d] xtest: 0x%08x\n", __FILE__, __LINE__, _xtest());

#define LBTREE_RTM_CHECK_TEST(x)  do { \
      if (_xtest() != (x)) fprintf(stderr, "[%s|%d] expect xtest: 0x%08x, but: 0x%08x \n", __FILE__, __LINE__, (x), _xtest()); \
   } while(0);

#define LBTREE_RTM_SHOW_REG()   do {   \
      int eax_value; \
      asm("\t movl %%eax,%0" : "=r"(eax_value));   \
      fprintf(stderr, "[%s|%d] eax: 0x%08x\n", __FILE__, __LINE__, eax_value);  \
   } while(0);
...
#define lbtree_min(x,y)    ((x)<=(y) ? (x) : (y))
#define lbtree_max(x,y)    ((x)<=(y) ? (y) : (x))

// compute ceiling(x/y) and floor(x/y)
#define ceiling(x, y)  (((x) + (y) - 1) / (y))
#define floor(x, y)    ((x) / (y))

#define lbtree_swap(x, y) \
